        ...
        public IQueryable<ApplicationListViewModel> FilterApplication(ApplicationFilterModel applicationFilterModel)
        {
            ...
            if (applicationFilterModel != null)
            {
                ...
                if (applicationFilterModel.LocalPlanningAuthorities != null)
                    queriedApplications = queriedApplications.Where(x => applicationFilterModel.LocalPlanningAuthorities.Select(x => x.Name).Contains(x.AddressData.Admin_district));

                if (applicationFilterModel.ApplicationTypes != null)
                    queriedApplications = queriedApplications.Where(x => applicationFilterModel.ApplicationTypes.Select(x => x.Id).Contains(x.ApplicationType)); 
